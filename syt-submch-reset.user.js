...
(function () {
  ...
  async function createCodePlateTransferFile(values) {
    const normalized = assertCodePlateTransferValues(values);
    const zipApi = typeof JSZip !== 'undefined' ? JSZip : globalThis.JSZip;
    if (!zipApi) throw new Error('官方模板处理组件加载失败，请刷新页面后重试');
    const zip = await withTimeout(
      zipApi.loadAsync(CODE_PLATE_TEMPLATE_BASE64, { base64: true }),
      10000,
      '读取内嵌官方 Excel 模板超时，请刷新页面后重试',
    );
    const sheetEntry = zip.file('xl/worksheets/sheet1.xml');
    const sharedStringsEntry = zip.file('xl/sharedStrings.xml');
    if (!sheetEntry || !sharedStringsEntry) throw new Error('官方模板文件结构不完整');
  ...
  function createPanel() {
    ...
    confirmCodePlateTransferButton.addEventListener('click', async () => {
      ...
      if (errorMessage) {
        ...
        setCodePlateTransferStatus('failure', errorMessage);
        return;
      }
      setCodePlateTransferError('');
      ...
      try {
        ...
        setCodePlateTransferError(message);
